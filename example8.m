   ...
   for s_e = 1:elem.n_sides;
       if ~elem.side(s_e).on_boundary;
            ...
            for i = 1:length(qps);
                ...
                if dot(v,n) >= 0;
                    Ken(de,dn) = Ken(de,dn) + Ws(i)*Np(qps(i))'*Nm(qps(i))*dot(v,n)*side_e.detJ();
                    Knn(dn,dn) = Knn(dn,dn) + Ws(i)*Nm(qps(i))'*Nm(qps(i))*dot(v,n)*side_e.detJ();
                else
                    Kne(dn,de) = Kne(dn,de) + Ws(i)*Nm(qps(i))'*Np(qps(i))*dot(v,n)*side_e.detJ();
                    Kee(de,de) = Kee(de,de) + Ws(i)*Np(qps(i))'*Np(qps(i))*dot(v,n)*side_e.detJ();  
                end
            end
